...
async function fetchDeepInfraImageModels(strict: boolean): Promise<ImagesModel<"deepinfra-images">[]> {
	try {
		...
		if (strict && models.length === 0) throw new Error("DeepInfra API returned no usable image models");
...
async function main(): Promise<void> {
	...
	// Independent catalog fetches; each fetcher returns [] on failure unless --strict is set.
	const [openrouter, deepinfra] = await Promise.all([
		fetchOpenRouterImageModels(strict),
		fetchDeepInfraImageModels(strict),
	]);
